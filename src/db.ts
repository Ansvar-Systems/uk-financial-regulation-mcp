/**
 * SQLite database access layer for the FCA Handbook MCP server.
 *
 * Schema mirrors the FCA Handbook structure:
 *   - sourcebooks        — top-level FCA Handbook sourcebook directories (SYSC, COBS, etc.)
 *   - provisions         — individual rules, guidance, evidential provisions, and directions
 *   - enforcement_actions — FCA Final Notices and enforcement actions
 *
...
export function getProvision(
  ...
  // Fuzzy fallback: strip trailing type suffix (R/G/E/D) if not found exact
  const bare = reference.replace(/[RGED]$/i, "").toUpperCase();
  return (
    (db
      .prepare(
